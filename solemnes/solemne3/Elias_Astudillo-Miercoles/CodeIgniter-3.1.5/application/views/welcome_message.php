<?php
defined('BASEPATH') OR exit('No direct script access allowed');
?>
    <!DOCTYPE html>
    <html>

    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <link rel="stylesheet" href="/CodeIgniter-3.1.5/assest/bootstrap-3.3.7-dist/css/bootstrap.min.css">
        <link rel="stylesheet" href="/CodeIgniter-3.1.5/assest/bootstrap-3.3.7-dist/css/bootstrap-theme.min.css">
        <link rel="stylesheet" href="/CodeIgniter-3.1.5/assest/EstiloLogin.css">
        <title>Monsters inc</title>
    </head>

    <body>
        <div class="loginPrincipal">
            <h1>Ingresa tu cuenta</h1><br>
            <form action="<?php echo site_url(" ")?>" method="get">
                <input type="text" name="user" placeholder="Correo" required>
                <input type="password" name="pass" placeholder="Constraseña" required>
                <input type="submit" name="login" class="boton-ingresar" value="Ingresar" formaction="">
            </form>
            <div class="ayuda">
                Bienvenido a Monsters Inc -
                <a href="RegistroCuenta.php"> Registrate</a>
            </div>

        </div>
    </body>

    </html>
